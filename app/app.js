...
/* Сетевая активность: полоска сверху экрана и «крутилки» кнопок ждут
   реального окончания запросов, а не таймера. Фоновые опросы (quiet) не считаются. */
const net = {
  n: 0,
  idleWaiters: [],
  bar: null,
  ensureBar() {
    if (this.bar || !document.body) return this.bar;
    this.bar = document.createElement('div');
    this.bar.className = 'net-bar';
  ...
  start() {
    this.n++;
    // полоска появляется, только если запрос дольше 200 мс — без мигания
    clearTimeout(this.tm);
    this.tm = setTimeout(() => {
      const b = this.n && this.ensureBar();
      if (b) { b.classList.remove('is-done'); b.classList.add('is-on'); }
    }, 200);
  },
  end() {
    ...
    if (b) {
      ...
      setTimeout(() => { if (!this.n) b.classList.remove('is-on', 'is-done'); }, 380);
    }
    const w = this.idleWaiters.splice(0);
    w.forEach(fn => { try { fn(); } catch (e) {} });
  },
  /** Промис, который разрешится, когда все запросы закончатся. */
  idle() { return this.n ? new Promise(r => this.idleWaiters.push(r)) : Promise.resolve(); },
};

/* Фоновые опросы (статус ответа чата, прогресс генерации) не должны
   дёргать полоску каждые 3 секунды. */
const NET_QUIET = [/\/chat\/\d+\/message\//, /\/projects\/\d+\/tasks$/, /\/health/, /\/auth\/me/];

async function req(path, opts = {}) {
  const quiet = opts.quiet || NET_QUIET.some(r => r.test(path));
  if (!quiet) net.start();
  try { return await reqRaw(path, opts); }
  finally { if (!quiet) net.end(); }
}
...
async function reqRaw(path, opts = {}) {
  ...
  const { noAuthRedirect, quiet: _q, headers: extraHeaders, ...fetchOpts } = opts;
  // FormData сам выставляет Content-Type с boundary — свой заголовок ставить нельзя.
  ...
  let response;
  try {
    // headers идут ПОСЛЕ ...fetchOpts: иначе headers из opts затирали токен (401).
...
const api = {
  ...
  // mid — id конкретного ответа; без него берётся последний ответ с каркасом.
  chatExtractFrame: (id, mid) => req(`/chat/${id}/frame` + qs({ message_id: mid }), { method: 'POST' }),
  ...
  /* Проверка перед генерацией: есть ли техзадания у строк в очереди. */
  ...
  // Документы проекта (PDF, DOCX, XLSX и т.д.)
  ...
  listLeads:    (p)      => req('/leads' + qs(p || {})),
  leadsStats:   ()       => req('/leads/stats'),
  getLead:      (id)     => req(`/leads/${id}`),
  updateLead:   (id, d)  => req(`/leads/${id}`, { method: 'PATCH', body: JSON.stringify(d) }),
  createLead:   (d)      => req('/leads/manual', { method: 'POST', body: JSON.stringify(d) }),
  editLead:     (id, d)  => req(`/leads/${id}/fields`, { method: 'PATCH', body: JSON.stringify(d) }),
  deleteLead:   (id)     => req(`/leads/${id}`, { method: 'DELETE' }),
  scanDomain:   (d)      => req('/scan', { method: 'POST', body: JSON.stringify(d) }),
  getScan:      (domain) => req('/scan' + qs({ domain })),
  lastScans:    ()       => req('/scans'),
};
...
const CHECK_INFO = {
  ...
  min_body:               ['Страница не пустая', '5.4'],
...
/* Справочное окно: заголовок + готовый HTML внутри. Одна кнопка «Понятно». */
function infoModal(opts) {
  const o = Object.assign({ title: 'Справка', html: '', close: 'Понятно', icon: 'info' }, opts || {});
  const wrap = document.createElement('div');
  wrap.className = 'modal-wrap';
  wrap.innerHTML =
    `<div class="scrim"></div>` +
    `<div class="modal is-lg" role="dialog" aria-modal="true">` +
      `<div class="modal-head">` +
        `<div class="modal-ico info">` + ico(o.icon, 19) + `</div>` +
        `<div class="modal-title">${escHtml(o.title)}</div>` +
      `</div>` +
      `<div class="modal-body" style="max-height:66vh;overflow:auto">${o.html}</div>` +
      `<div class="modal-foot"><button class="btn btn-primary" data-ok>${escHtml(o.close)}</button></div>` +
    `</div>`;
  document.body.appendChild(wrap);
  const done = () => { document.removeEventListener('keydown', onKey); wrap.remove(); };
  const onKey = e => { if (e.key === 'Escape') done(); };
  wrap.querySelector('[data-ok]').onclick = done;
  wrap.querySelector('.scrim').onclick = done;
  document.addEventListener('keydown', onKey);
  setTimeout(() => wrap.querySelector('[data-ok]').focus(), 40);
}
 ...
 *  Иконка-кнопка (или keepIcon) не прячется — крутится сама иконка.
 *  Анимация держится минимум 450 мс, иначе на быстрых ответах её не видно. */
async function withBusy(btn, fn, opt = {}) {
  if (!btn) return fn();
  const wasDisabled = btn.disabled;
  const iconMode = opt.keepIcon || btn.classList.contains('icon-btn');
  const cls = iconMode ? 'is-spin' : 'is-busy';
  const t0 = Date.now();
  ...
  try { return await fn(); }
    ...
    if (!iconMode) { btn.classList.add('is-done'); setTimeout(() => btn.classList.remove('is-done'), 500); }
  }
}
...
/* Ось 1 — тип страницы. Что человек хочет сделать. Выбирается.
   Он же определяет макет: третьего независимого выбора больше нет. */
const LIB_PAGE_TYPES = [
  { id: 'catalog', icon: 'table', name: 'Каталог',
    when: 'Есть прайс или выгрузка каталога', ex: 'арматура 12 мм купить в Казани',
    design: 'Цена и наличие сверху, таблица характеристик, вопросы' },
  { id: 'service_geo', icon: 'radar', name: 'Услуга и город',
    when: 'Услуга повторяется по городам', ex: 'замена ремня ГРМ Solaris Казань',
    design: 'Местные факты, своя цена и срок, таблица, вопросы' },
  { id: 'choice', icon: 'layout', name: 'Подбор',
    when: 'Варианты сравниваются по общим параметрам', ex: '09Г2С или Ст3',
    design: 'Таблица сравнения, вывод, вопросы' },
  { id: 'calc', icon: 'bolt', name: 'Расчёт',
    when: 'Есть числа и понятная формула', ex: 'вес трубы 57×3,5',
    design: 'Результат крупно, шаги расчёта, исходные данные' },
  { id: 'reference', icon: 'list', name: 'Справка',
    when: 'Данных клиента нет, факты сводим из источников', ex: 'значение карты Шут',
    design: 'Таблица фактов, источники, вывод, вопросы' },
];

/* Ось 2 — источник уникальности. НЕ выбирается: выводится из данных.
   Список нужен только для подписей в интерфейсе. Порядок = сила. */
const LIB_VALUE_SOURCES = [
  { id: 'own_data', icon: 'target', name: 'Собственные данные',
    gives: 'Цена, наличие, сроки, свои работы — этого нет ни у кого',
    needs: 'Колонка с ценой, наличием, сроком или своими фактами' },
  { id: 'merge', icon: 'layout', name: 'Сведение данных',
    gives: 'Разрозненные факты собраны в одну таблицу',
    needs: 'Три и более фактовых колонки' },
  { id: 'compute', icon: 'bolt', name: 'Вычисление',
    gives: 'Цифра, которую читатель считал бы руками',
    needs: 'Две и более числовые колонки' },
  { id: 'live', icon: 'clock', name: 'Актуальность',
    gives: 'Данные обновляются сами',
    needs: 'Живой источник: Sheets или ссылка с автообновлением' },
  { id: 'expertise', icon: 'radar', name: 'Опыт клиента',
    gives: 'Практика и ограничения, которых нет в данных',
    needs: 'Заполненный блок опыта в паспорте' },
];

/* Совместимость со старым кодом: где-то мог остаться LIB_STRATEGIES. */
const LIB_STRATEGIES = LIB_VALUE_SOURCES;
...
function renderGateConfig() {
  ...
  const groups = [
    ['5.1', 'Техническая исправность'],
    ['5.3', 'Защита от дублей'],
    ['5.4', 'Дополнительные проверки'],
  ];
  ...
  if (note) {
    note.innerHTML =
      '<b>Как это работает.</b> Техническая ошибка или дубль блокируют страницу. ' +
      'Итоговый балл помогает решить: выгружать, переделать или отклонить. ' +
      'Проверка фактов сюда не входит: качество исходных данных оценивается раньше, на шаге ИИ-анализа.';
  }
...
/* --------------------------------------------------------------------------
   ОТКЛИК НА НАЖАТИЯ — работает на всех страницах кабинета
   1) «волна» от точки нажатия на любой кнопке;
   2) иконки «обновить / пересчитать» крутятся, пока идут запросы.
   -------------------------------------------------------------------------- */
(function mountClickFx() {
  const BTN = '.btn, .icon-btn, .seg button, .nav-item, .set2-item, [role="button"]';
  const SPIN_ICONS = ['refresh', 'redo'];
  ...
  document.addEventListener('pointerdown', e => {
    ...
    // обёртка с overflow:hidden — саму кнопку не обрезаем (бейджи, фокус)
    const holder = document.createElement('span');
    holder.className = 'fx-ripple-box';
    ...
    w.addEventListener('animationend', () => holder.remove());
    setTimeout(() => holder.remove(), 900);
  }, { passive: true });

  document.addEventListener('click', e => {
    const b = e.target.closest('button, a, [role="button"]');
    if (!b || b.classList.contains('is-spin') || b.classList.contains('is-busy')) return;
    const icoEl = b.querySelector('[data-ico], [data-ico-sm]');
    const name = icoEl && (icoEl.dataset.ico || icoEl.dataset.icoSm);
    if (!SPIN_ICONS.includes(name)) return;
    // ждём, пока обработчик успеет отправить запросы, потом — пока они идут
    b.classList.add('is-spin');
    const t0 = Date.now();
    setTimeout(async () => {
      await net.idle();
      const left = 700 - (Date.now() - t0);
      if (left > 0) await new Promise(r => setTimeout(r, left));
